...
public class UFOAbduction : MonoBehaviour {
    ...
    void Update() {
        ...
        float triggerVal = controller._controller.TriggerValue;
        if (triggerVal < Mathf.Epsilon) {
            controller.triggerEnabled = true;
        }
        // Abduct 
        if ((triggerVal > TRIGGER_MIN) && controller.triggerEnabled) {
            ...
            if (target) {
                ...
                // Haptic
                controller.haptic_tick(MLInputControllerFeedbackIntensity.Low);
    ...
    void destroyTarget() {
        ...
        controller.haptic_bump(MLInputControllerFeedbackIntensity.High);
